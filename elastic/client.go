...
func NewModel[T Elastic](client *elasticsearch.Client) *Model[T] {
	var t T
	var config = (t).Config()
...
func (m *Model[T]) Aggregate(query kitty.M) *Aggregation {

	var result Aggregation

	var dslBts, err = json.Marshal(query)
	if err != nil {
		result.Error = err
		return &result
	}
	...
	if res.IsError() {
		result.Error = errors.New(res.String())
		return &result
	}

	return &Aggregation{Reader: res.Body, Error: nil}
}
